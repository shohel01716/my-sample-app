...
import 'package:face_net_authentication/model/header_model.dart';
import 'package:face_net_authentication/storage/session.dart';
...
class CoreService extends GetConnect {
  ...
  Future<dynamic> postWithoutAuth(
    ...
    if (data.hasError) {
      Future.delayed(const Duration(milliseconds: 500), () {
        Get.snackbar(NetworkLabel.attention, NetworkLabel.connectionErrorMessage,
  ...
  Future<dynamic> putWithoutAuth(
    ...
    // var data = await post(url, FormData(body!)); //This is used for passing the body as form data
    log("response body: ${data.body.toString()}");
